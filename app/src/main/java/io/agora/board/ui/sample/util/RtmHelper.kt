...
class RtmHelper {
    ...
    init {
        ...
        val config = RtmConfig.Builder(
            ConfigLoader.forgeRtmAppId,
            ConfigLoader.currentUser.userId,
    ...
    suspend fun login() = suspendCoroutine { cont ->
        rtmClient.login(ConfigLoader.currentUser.rtmToken, object : ResultCallback<Void> {
    ...
    companion object {
        private var instance: RtmHelper? = null

        fun getInstance(): RtmHelper {
            if (instance == null) {
                instance = RtmHelper()
            }
            return instance!!
